...
def get_embeddings(
    ...
    # Combine embeddings (train + test) and create ID to index mapping
...
class AdvancedPhysioEncoder(nn.Module):
    ...
    def forward(self, id_features, et_features, hr_features, eeg_features):
        ...
        conditioned_physio = combined_physio
        if self.use_subject_ids:
            # Modulate combined physio features with the subject profile.
...
class BiosignalCrossAttention(nn.Module):
    ...
    def forward(
        ...
        V_physio = self.physio_encoder(
            id_features, et_features, hr_features, eeg_features
        )
...
class Gemini(nn.Module):
    def __init__(
        ...
        super().__init__()
        embeddings, _, id_to_embedding_idx = get_embeddings(
    ...
    def forward(
        ...
        if (
            self.use_sensorial
            and id_features is not None
            and et_features is not None
            and hr_features is not None
            and eeg_features is not None
            and physio_mask is not None
        ):
            features = self.physio_fusion(
                features,
                id_features,
                et_features,
                hr_features,
